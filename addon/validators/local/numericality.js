...
export default Base.extend({
  init: function() {
    ...
    if (this.options.messages === undefined || this.options.messages.numericality === undefined) {
      ...
      this.options.messages.numericality = this.get("messages").render('notANumber', this.options);
    }

    if (this.options.onlyInteger !== undefined && this.options.messages.onlyInteger === undefined) {
      this.options.messages.onlyInteger = this.get("messages").render('notAnInteger', this.options);
    }
    ...
    for(index = 0; index < keys.length; index++) {
      ...
      if (prop !== undefined && this.options.messages[key] === undefined) {
        ...
        this.options.messages[key] = this.get("messages").render(key, this.options);
